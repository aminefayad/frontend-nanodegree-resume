...
// This is the fun part. Here's where we generate the custom Google Map for the website.  See the documentation below for more details.  https://developers.google.com/maps/documentation/javascript/reference
...
function initializeMap() {
    ...
    // returns an array of every location string from the JSONs written for bio, education, and work.
    ...
    function createMapMarker(placeData) {
        ...
        // helper windows that open when you click or hover over a pin on a map. They usually contain more information about a location.
        var infoWindow = new google.maps.InfoWindow({
            content: '<div class="map-tooltip"><img src="images/Building.png" align="absmiddle" />' + name + '</div>'
        });
        ...
        bounds.extend(new google.maps.LatLng(lat, lon)); // this is where the pin actually gets added to the map.  bounds.extend() takes in a map location object
    ...
    function pinPoster(locations) {
        var service = new google.maps.places.PlacesService(map); // creates a Google place search service object. PlacesService does the work of actually searching for location data.
        for (var place in locations) { // Iterates through the array of locations, creates a search object for each location
            var request = { query: locations[place] }; // the search request object
            service.textSearch(request, callback);  // Actually searches the Google Maps API for location data and runs the callback function with the search results after each search.
        }
    }

    window.mapBounds = new google.maps.LatLngBounds(); // Sets the boundaries of the map based on pin locations
    locations = locationFinder(); // locations is an array of location strings returned from locationFinder()
